...
require APPPATH . '/libraries/Api_Log.php';
...
abstract class REST_Controller extends CI_Controller {
     ...
     * @param $message
     */
    protected function returnError($code,$message,$data=array())
    {
        $out = array(
            'code'  =>  $code,
            'msg'   =>  $message,
        ...
        $this->_log->write_log($this->post('method'),json_encode($_POST),'error');
        $this->_log->write_log($this->post('method'),json_encode($out),'error');
    ...
    protected function returnSuccess($data)
    {
        ...
        $this->_log->write_log($this->post('method'),json_encode($out));
        $this->_log->write_log($this->post('method'),"finished");
        $this->set_response($out, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    }
